...
| **512 × 16** | **4.711 / 4.530** | 数量增大到 512 仍有小幅收益 |
...
| 精巧化尝试 | 结果 | 简单版 |
|---|---|---|
| 迭代 G（在新模型上重测梯度再重建） | 4.952/4.860，结果变差 | 一次测量 4.944/4.834 |
| 完整 G（k=64 特征子空间 + 对角） | 4.841/4.796，结果变差 | 对角 G 4.825/4.768 |
| S 份额加大到 32%（度量加持下"S 更聪明了"） | 4.969/4.882，结果变差 | 21% 仍最优 |
| 按损失分布图定向的矩阵类型×深度分配 | 5.141/5.050，结果变差 | Fisher 阻尼图 |
