...
### 1. 稀疏残差：W ≈ AB + S，第一次凿穿账 1

账 1（截断税，~2 nat）的架构根源是内容通路的超位置重尾谱——低秩在数学上表达不了 incoherent 的重尾结构。但**稀疏项可以**。这是文献中证据最一致的换范式方向（OATS/HASSLE-free/LoSparse：等预算下 S+LR 一致优于纯 LR，且差距随压缩率放大）。
...
第四篇的税收地图留了一个反常没解释：qkv@late 在白化能量下"几乎无损"（保留 94-98%），却是最大的税项（+0.56）。反常的谜底是：**我们的截断在错误的目标函数下最优**。白化 SVD 最小化的是该层输出的 L2 误差（Eckart-Young），但输出误差到最终 loss 的映射高度各向异性——q/k 的某些方向误差被 softmax 与下游放大数十倍，down 的很多大能量方向却被残差流稀释。
...
第四篇提出过"小红利不叠加定律"（0.02 级的改进合并时互相侵蚀）。这次的组合实验修正了它：

| 配置（等预算） | held-out |
|---|---|
| 基线 | 5.114 / 5.023 |
| 只加稀疏残差 | 5.073 / 4.978（−0.04） |
...
| 统计配置 | held-out | 备注 |
|---|---|---|
| 32 批 × 1 shard | 4.944 / 4.834 | 组合配方基线 |
| 128 × 1 | 4.825 / 4.768 | 数量 ×4 |
| 128 × 4 shards | 4.774 / 4.619 | **同数量、多样性 ×4** |
| 256 × 8 | 4.738 / 4.555 | 双倍 |
| 256 × 16 | 4.742 / 4.558 | 多样性饱和于 8 shards |
| **512 × 16** | **4.711 / 4.530** | 数量在 512 仍小额付钱 |

从 4.944 到 4.711——**校准数据工程独立贡献了 ~0.23 nat**，与任何算法改进同量级。两个可迁移的定量结论：

1. **多样性 ≠ 数量**。128 批不变、从 1 shard 换 4 shards：−0.05/−0.15。这是偏差-方差分解里的**偏差项**——同 shard 加数据只压采样方差，分布偏色一分不动。窗口间改善的不对称（−0.05 vs −0.15）直接暴露了单 shard 校准与某个验证窗口"口味相近"的偏差本质。
2. 两轴各自饱和：多样性 8 shards、数量 ~512 批（26 万→105 万 token 位置）。饱和点大约在"统计噪声降到单个算法改进量级（±0.01）"处，符合直觉。
...
| 精巧化尝试 | 结果 | 简单版 |
|---|---|---|
| 迭代 G（在新模型上重测梯度再重建） | 4.952/4.860，回退 | 一次测量 4.944/4.834 |
| 完整 G（k=64 特征子空间 + 对角） | 4.841/4.796，回退 | 对角 G 4.825/4.768 |
| S 份额加大到 32%（度量加持下"S 更聪明了"） | 4.969/4.882，回退 | 21% 仍最优 |
| 税收地图定向的家族×深度分配 | 5.141/5.050，回退 | Fisher 阻尼图 |

机制各不相同（迭代破坏自洽链、特征子空间对 24 batch 过拟合、oracle 收益不等于真实边际价值），但工程结论一致：**在校准数据有限的闭式世界里，参数量少的估计器赢**——对角赢完整、单遍赢迭代、阻尼赢全量。每个"更准的模型"都要用同样的 24-512 个 batch 去喂，多出来的自由度全变成噪声。
...
1. **度量是一等公民。** 逼近算法的"最优性"永远是相对某个度量的；换成 loss 感知的度量后，此前"榨干"的每个杠杆都重新有了油。检查任何压缩管线时，先问它的截断在什么度量下最优。
2. **校准数据是一个被忽略的工程对象。** 多样性和数量各自独立贡献，多样性优先（同预算下先加 shard 再加量），两轴的饱和点都可以用剂量实验便宜地测出。~0.23 nat 的免费收益在文献的标准配置里全部躺着没人捡。
3. **参数少的估计器赢**（闭式版奥卡姆）：对角 > 完整协方差，单遍 > 迭代，阻尼 > 全量执行。校准数据的信息量是硬约束，精巧化只是把它重新分配给噪声。
...
Background in one line: Qwen3-8B (2.11) → every linear replaced by rank-r factors (2.29B equal budget, 72% removed); method = layerwise trajectory-correcting regression + tax-free correctors + loss-aware allocation; part 4 endpoint 5.114/5.023.

### 1. Sparse Residuals: W ≈ AB + S, First Breach of Ledger 1

Ledger 1 (the ~2-nat truncation tax) is rooted in superposition heavy tails on the content pathway — structure low-rank factors mathematically cannot express. A **sparse term can**. It is also the literature's most consistently supported paradigm change (OATS/HASSLE-free/LoSparse: S+LR beats pure LR at equal budget, with the gap widening at higher compression).
...
| S share | held-out (both windows) |
|---|---|
| 0% (pure low-rank) | 5.114 / 5.023 |
| **21%** | **5.073 / 4.978** |
| 41% | 5.126 / 4.997 |
...
### 2. Changing the Ruler: Output-Sensitivity-Weighted Truncation
...
### 3. Super-Additive Stacking: Amending the Dividends Law

Part 4 proposed a "small dividends don't stack" law (0.02-level improvements erode each other on combination). The combination experiment this time amended it:

| Configuration (equal budget) | held-out |
|---|---|
| baseline | 5.114 / 5.023 |
| sparse residual only | 5.073 / 4.978 (−0.04) |
...
|---|---|---|
| 32 batches × 1 shard | 4.944 / 4.834 | combo-recipe baseline |
| 128 × 1 | 4.825 / 4.768 | quantity ×4 |
| 128 × 4 shards | 4.774 / 4.619 | **same quantity, diversity ×4** |
| 256 × 8 | 4.738 / 4.555 | double both |
| 256 × 16 | 4.742 / 4.558 | diversity saturates at 8 shards |
| **512 × 16** | **4.711 / 4.530** | quantity still pays mildly at 512 |

From 4.944 to 4.711 — **calibration-data engineering alone contributed ~0.23 nat**, on par with any single algorithmic idea. Two transferable quantitative findings:

1. **Diversity ≠ quantity.** Holding 128 batches fixed and going 1 shard → 4 shards: −0.05/−0.15. In bias-variance terms this is the **bias** term — more data from the same shard only shrinks sampling variance, leaving the distributional tint untouched. The asymmetry across validation windows (−0.05 vs −0.15) directly exposed that the single-shard calibration "tasted like" one window.
2. Each axis saturates on its own: diversity at 8 shards, quantity at ~512 batches (0.26M → 1.05M token positions). Saturation arrives roughly when statistical noise drops to the size of one algorithmic improvement (±0.01), as intuition suggests.

### 5. The Failure Ledger: Simple-Robust Always Wins
...
|---|---|---|
| Iterated G (remeasure gradients on the new model, rebuild) | 4.952/4.860, regression | one measurement: 4.944/4.834 |
| Full G (k=64 eigen-subspace + diagonal) | 4.841/4.796, regression | diagonal G: 4.825/4.768 |
| S share raised to 32% ("S is smarter now") | 4.969/4.882, regression | 21% remains optimal |
| Tax-map-guided family×depth allocation | 5.141/5.050, regression | damped Fisher map |

The mechanisms differ (iteration breaks the self-consistent chain; the eigen-subspace overfits 24 calibration batches; oracle gains ≠ real marginal value), but the engineering conclusion is uniform: **in the calibration-limited closed-form world, the estimator with fewer parameters wins** — diagonal over full, single-pass over iterated, damped over full-strength. Every "more accurate model" must be fed by the same 24-512 batches; the extra degrees of freedom all turn into noise.
...
1. **The metric is a first-class citizen.** An approximation algorithm's "optimality" is always relative to a metric; switching to a loss-aware one refilled every previously "exhausted" lever. When auditing any compression pipeline, first ask under which metric its truncation is optimal.
2. **Calibration data is a neglected engineering object.** Diversity and quantity contribute independently; diversity first (add shards before adding volume); both saturation points are cheap to measure with dose-response runs. ~0.23 nat of free gains lie untouched in the literature's standard setup.
3. **Fewer-parameter estimators win** (the closed-form Occam): diagonal > full covariance, single-pass > iterated, damped > full-strength. The information in the calibration set is the hard constraint; sophistication merely reallocates it to noise.
