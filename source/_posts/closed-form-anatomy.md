...
**① 截断税（最大头）**：即使每个子层都收到教师的干净输入，rank-384 的局部截断误差之和也要付约 2 nat。为什么这么贵？白化谱给出答案——下表是 rank 384 下各类矩阵保留的能量（白化坐标里奇异值平方和的占比，即最优截断能留住多大比例的"有效信号"）：
...
**其一：截断税几乎全部集中在后半段。** 只把 block 18-35 的输入换干净，loss 就到了完整 oracle 的水平——意味着前半段的局部截断误差对 logits 的直接贡献只有约 0.01 nat，它的全部危害走"制造漂移"这条间接通道；而漂移是（部分）可矫正的，后半段的截断税则无药可救。**推论：边际 rank 在后半段的价值远高于前半段。** 我们此前试过的恰好是反方向（front-load，5.38 失败——当时的理论"减少流进 block 16 的漂移"错得离谱）。
