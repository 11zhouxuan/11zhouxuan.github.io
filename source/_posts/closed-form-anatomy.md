...
三笔账对应三把不同的锁。本篇的四场战役全部发生在账 1 和账 2 的边界上。
...
2. **非均匀 rank 分配是文献中最一致的红利来源**（Dobi-SVD、SVD-LLM v2、D-Rank、UniRank、AIR、LACE-SVD 六篇独立工作同向）：高压缩下"分配比分解本身更重要"。这直接触发了战役一。

### 3. 战役一：loss 敏感度驱动的 rank 分配——方向对，但要阻尼
...
### 5. 战役三：rms-lift 非线性矫正器——机制证实，红利归零
...
### 6. 战役四：oracle 分解——税收的深度结构与 back-load
